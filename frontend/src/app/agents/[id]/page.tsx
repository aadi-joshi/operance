...
import {
  ...
  Database, Shield, FileText, Code2, BarChart3, Cpu,
  type LucideIcon,
} from "lucide-react";
...
export default function AgentDetailPage() {
  ...
  const meta = CAPABILITY_META[agent.capability] || { color: "#0052ff", label: "Agent" };
  const CapIcon = CAPABILITY_ICONS[agent.capability] || Cpu;
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="rounded-xl border border-border-default bg-bg-card p-6 mb-5">
            <div className="flex items-start gap-4">
              ...
              >
                <CapIcon size={26} style={{ color: meta.color }} />
              </div>
          ...
          <div className="grid grid-cols-3 gap-4 mb-5">
            {[
              ...
              <div key={stat.label} className="rounded-xl border border-border-default bg-bg-card p-4">
                <div className="text-[22px] font-bold text-text-primary font-mono tabular-nums">{stat.value}</div>
